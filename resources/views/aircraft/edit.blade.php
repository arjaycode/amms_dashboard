<x-layout bodyClass="g-sidenav-show  bg-gray-200">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
      <!-- Navbar -->
      <x-navbars.navs.auth titlePage="Edit Aircraft"></x-navbars.navs.auth>
      <!-- End Navbar -->
      <div class="container-fluid py-4">
          <x-aircraft.content_nav></x-aircraft.content_nav>
            <div class="mx-3">
              <h4>Edit Page - {{ $aircraft->model }}</h4>
              {{-- Error Messages  --}}
              @if ($errors->any())
              <div class="container">
              ...
              <form action="{{ route('aircraft.update', $aircraft->id) }}" method="POST">
                ...
                <div class="container d-flex flex-row gap-4">
                  ...
                  <div class="mb-3 col">
                    ...
                    value="{{ old('tail_number') ?? $aircraft->tail_number }}">
                  </div>
                  {{-- Manufacturer --}}
                  ...
                  <div class="mb-3 col">
                    ...
                    <select name="manufacturer_id" id="manufacturer_id" class="form-select bg-white px-1" required>
                      ...
                      @foreach($manufacturers as $manufacturer)
                        <option value="{{ $manufacturer->id }}" {{ $manufacturer->id == old('manufacturer_id', $aircraft->manufacturer_id) ? 'selected' : '' }}>{{ $manufacturer->name }}</option>
                      @endforeach
                    </select>
                ...
                <div class="container d-flex flex-row gap-4">
                  ...
                  <div class="mb-3 col">
                    ...
                    <input type="text" name="model" id="model" class="form-control bg-white px-1" value="{{ old('model', $aircraft->model) }}">
                  </div>
                  {{-- Year of Manufacture --}}
                  <div class="mb-3 col">
                    ...
                    min=1900 max="{{ date('Y') }}"
                    value="{{ old('year_of_manufacture', $aircraft->year_of_manufacture) }}"
                    >
                  </div>
                </div>
                <div class="container d-flex flex-row gap-4">
                  ...
                  <div class="mb-3 col">
                    ...
                    min="0"
                    step="0.01"
                    value="{{old('total_flight_hours', $aircraft->total_flight_hours) }}">
                  </div>
                  {{-- Total Landings  --}}
                  <div class="mb-3 col">
                    ...
                    min="0"
                    value="{{old('total_landings', $aircraft->total_landings) }}">
                  </div>
                </div>
                <div class="container d-flex flex-row gap-4">
                  ...
                  <div class="mb-3 col">
                    ...
                    value="{{ old('last_maintenance_date', $aircraft->last_maintenance_date) }}">
                  </div>
                  ...
                  <div class="mb-3 col">
                    ...
                    value="{{ old('next_maintenance_due', $aircraft->next_maintenance_due) }}">
                  </div>
                </div>
                  ...
                  <div class="mb-3 col-4">
                    ...
                    <select name="status_id" id="status_id" class="form-select bg-white px-2">
                      @foreach($statuses as $status)
                        <option value="{{ $status->id }}" {{ old('status_id', $aircraft->status_id) == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
